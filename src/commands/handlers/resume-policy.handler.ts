...
export class ResumePolicyHandler
    implements ICommandHandler<ResumePolicyCommand, ResumePolicyResult> {
    private readonly logger = new Logger(ResumePolicyHandler.name);

    constructor(private readonly policiesRepo: PoliciesRepo) { }

    async execute(command: ResumePolicyCommand): Promise<CommandResult<ResumePolicyResult>> {
