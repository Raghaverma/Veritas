...
export class SuspendPolicyHandler
    implements ICommandHandler<SuspendPolicyCommand, SuspendPolicyResult> {
    private readonly logger = new Logger(SuspendPolicyHandler.name);

    constructor(private readonly policiesRepo: PoliciesRepo) { }

    async execute(command: SuspendPolicyCommand): Promise<CommandResult<SuspendPolicyResult>> {
        ...
        await this.policiesRepo.update(policy, result.events, payload.expectedVersion);
