...
export class RevokePolicyHandler
    implements ICommandHandler<RevokePolicyCommand, RevokePolicyResult> {
    private readonly logger = new Logger(RevokePolicyHandler.name);

    constructor(private readonly policiesRepo: PoliciesRepo) { }

    async execute(command: RevokePolicyCommand): Promise<CommandResult<RevokePolicyResult>> {
