...
export class ActivatePolicyHandler
    implements ICommandHandler<ActivatePolicyCommand, ActivatePolicyResult> {
    private readonly logger = new Logger(ActivatePolicyHandler.name);

    constructor(private readonly policiesRepo: PoliciesRepo) { }

    async execute(command: ActivatePolicyCommand): Promise<CommandResult<ActivatePolicyResult>> {
